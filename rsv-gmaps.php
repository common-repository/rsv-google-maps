...
function rsv_gmaps_enqueued_assets() {
	...
	// wp_enqueue_script( 'j-script', 'http://maps.google.com/maps/api/js', array(), '1.0', false );
}
...
function rsv_gmaps_wpt_save_events_meta($post_id, $post) {
	...
	$coordinates = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?address=' . urlencode($address) . '&sensor=true&key='.get_option('rsv_api_of_map'));
...
$rsv_api_of_map=get_option('rsv_api_of_map');
...
$output.="<div id='map' style='width: 100%; height: 400px;'></div>";
 ...
 $output.= '<script src="https://maps.googleapis.com/maps/api/js?key='.$rsv_api_of_map.'&callback=initMap"
 async defer></script>';
return $output;
...
function rsv_gmaps_plugin_options() {
 ...
 if($_POST['rsv_data_save']){
	$rsv_api_of_map = $_POST['rsv_api_of_map'];   
	update_option('rsv_api_of_map',$rsv_api_of_map);
